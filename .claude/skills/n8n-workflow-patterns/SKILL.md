...
**Every node gets a PascalCase name.** `ResolveRequester`,
`CallGovernedDataPlatform`, `RaiseServiceDeskTicket`. Not `Resolve requester`,
not `resolve_requester`, not `Resolve Requester`.

Two reasons, both mechanical rather than stylistic:

- **Tool nodes derive their tool name from the node name.** `toolWorkflow` at
  typeVersion 2.2 dropped the separate `name` parameter, so the name the model
  sees is the node name, sanitised down to letters, numbers and underscores. A
  name with spaces gets rewritten on the way through, which means what the model
  reads is not what you typed. PascalCase passes through unchanged.
- **Code nodes and expressions reference nodes by name.** `$('ResolveRequester')`
  needs no quoting; `$node["Resolve requester"]` does, and one wrong space or
  capital returns `undefined` with no error at all. See **n8n-expression-syntax**
  `COMMON_MISTAKES.md` sections 4 and 5 for what that failure looks like.

Rules:

- PascalCase. No spaces, underscores, or hyphens.
- Still verb-first and specific. The convention constrains the shape, not the
  descriptiveness: `SearchCustomerDatabase`, never `Database`.
- Acronyms are treated as words: `CallCrmApi`, `ParseXmlPayload`.
  `ParseXMLAPIResponse` is the reason for this rule.
- Node names are case-sensitive wherever they are referenced. Renaming a node
  breaks every expression naming it, and n8n does not reliably fix them for you,
  so name it correctly on creation rather than tidying later.
- Sticky notes are exempt, since nothing references them. Prefix them `Sticky`
  and keep them readable.

**This applies to node names only.** Workflow names keep the verb-first prefix
convention (`Subworkflow: Parse RFC2822 date`) because they are searched by
humans and by `n8n_list_workflows`, never referenced from code. See
**n8n-subworkflows**.
...
- Name every node in PascalCase, verb-first and specific (see **Node naming**)
