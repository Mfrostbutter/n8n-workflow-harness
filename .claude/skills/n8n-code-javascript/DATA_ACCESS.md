...
> **Naming.** Nodes you create are named in PascalCase (`HttpRequest`,
> `GetToken`), so `$('GetToken')` and `$node.GetToken.json` both work without
> quoting. Examples below that show spaces are referencing nodes that already
> exist under a name you did not choose; for those the bracket form is required.
> Convention in **n8n-workflow-patterns**, Node naming.
