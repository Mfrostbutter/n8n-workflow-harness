...
|---|---|---|
| Generic tool names (`tool1`, `doStuff`, `runQuery`) | Model can't tell which tool to pick — skips them or hallucinates params | PascalCase, verb-first, specific: `SearchCustomerDatabase`, `GenerateImageWithVeo` |
| Tool node named with spaces (`Search customer database`) | At `toolWorkflow` 2.2 the tool name IS the node name, sanitised to letters/numbers/underscores — the model sees something you did not write | PascalCase the node name. See **n8n-workflow-patterns**, Node naming |
...
- [ ] **Every tool** has a PascalCase, verb-first, specific name AND a real description
