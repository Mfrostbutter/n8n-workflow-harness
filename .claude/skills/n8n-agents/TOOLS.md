...
### Names: PascalCase, verb-first, specific

The tool name the model sees **is the node name**. `toolWorkflow` at typeVersion
2.2 dropped the separate `name` parameter, and n8n sanitises whatever is left to
letters, numbers and underscores. A name with spaces gets rewritten in transit, so
what the model reads is not what you typed. PascalCase survives untouched.

| Good | Bad | Why |
|---|---|---|
| `SearchCustomerDatabase` | `query` / `tool1` | Generic names say nothing |
| `GenerateImageWithVeo` | `imageGen` | Which generator? |
| `EditExistingImage` | `edit` | Edit what? |
| `SendSlackMessageToChannel` | `slack` | Name the action, not just the surface |
| `LookupUserByEmail` | `getUser` | Lookup how? |
| `QueryGovernedBusinessData` | `Query governed business data` | Spaces get sanitised; you no longer control the name |

Acronyms are treated as words: `CallCrmApi`, not `CallCRMAPI`. Full convention in
**n8n-workflow-patterns**, Node naming.

PascalCase constrains the shape, not the descriptiveness. A tool still needs to
say what it does; `Data` is as useless as `tool1`.
