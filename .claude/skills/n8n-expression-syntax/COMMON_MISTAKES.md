...
**How to stop it happening**: name nodes in PascalCase (`HttpRequest`,
`RespondToWebhook`) and the whole class of mistake disappears — `$('HttpRequest')`
and `$node.HttpRequest.json` both just work. The convention is in
**n8n-workflow-patterns**, Node naming. You still need the bracket form for nodes
you did not name, including anything n8n created for you.
