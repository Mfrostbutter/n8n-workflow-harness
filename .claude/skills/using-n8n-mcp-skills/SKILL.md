...
- **Node names are PascalCase.** `ResolveRequester`, not `Resolve requester`. Tool nodes
  hand the node name to the model, and code references it; spaces break both. Workflow
  names keep the readable verb-first prefix. See `n8n-workflow-patterns`, Node naming.
